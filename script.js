...
/**
 * 3. RENDERIZADO, BUSCADOR Y FAVORITOS
 */
function renderizar(lista) {
    ...
    lista.forEach(p => {
        ...
        const corazonIcon = isFav ? '❤️' : '♡';

        contenedor.innerHTML += `
            <div class="product-card">
                <div class="img-container">
                    <div class="fav-btn-item ${isFav ? 'active' : ''}" onclick="toggleFavorito('${p.id}')">
                        ${corazonIcon}
                    </div>
...
function toggleFavorito(id) {
    ...
    // Actualizamos solo los iconos sin recargar todo para mejor performance
    renderizar(inventarioCompleto); 
}

/**
 * 4. CARRITO Y NAVEGACIÓN
 */
        ...
        listaHtml.innerHTML += `
            <div class="cart-item">
                ...
                <div class="cart-info">
                    ...
                    <p class="cart-item-price">S/ ${p.precio.toFixed(2)}</p>
                </div>
                <button class="cart-remove" onclick="quitarCarrito(${i})">✕</button>
            </div>`;
    });
...
function toggleCart(force) {
    ...
    if(force === true) cart.classList.add('open');
    else if(force === false) cart.classList.remove('open');
...
window.onload = () => { obtenerDatos(); inicializarCarrusel(); };